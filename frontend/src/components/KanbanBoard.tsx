...
export default function KanbanBoard({ tasksByStatus, onStatusChange, onEdit, onDelete }: KanbanBoardProps) {
  const [dragging, setDragging] = useState<Task | null>(null);
  const [dragOver, setDragOver] = useState<TaskStatus | null>(null);

  const handleDragStart = (e: DragEvent, task: Task) => { setDragging(task); e.dataTransfer.effectAllowed = 'move'; };
  const handleDragOver = (e: DragEvent, colKey: TaskStatus) => { e.preventDefault(); setDragOver(colKey); };
  const handleDrop = (e: DragEvent, colKey: TaskStatus) => {
    e.preventDefault();
