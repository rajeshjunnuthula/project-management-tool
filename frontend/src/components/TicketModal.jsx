...
export default function TicketModal({ projectId, members, projects, ticket, onClose, onSaved }) {
  const isEdit = !!ticket;
  const { form, set } = useForm({
    title: ticket?.title || '',
    description: ticket?.description || '',
    project: projectId || ticket?.project?._id || ticket?.project || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSaved(data, !isEdit);
    } catch (err) { setError(err.response?.data?.message || 'Failed to save ticket'); }
          ...
          <div className={MODAL_BODY}>
            ...
            <div className={FORM_ROW}>
              <div className={FORM_GROUP}>
                ...
                <select value={form.type} onChange={e => set('type', e.target.value)} className={FORM_INPUT}>
