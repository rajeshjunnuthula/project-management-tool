...
export default function Milestones() {
  ...
  const filter = FILTERS.includes(searchParams.get('status')) ? searchParams.get('status') : 'all';
  const [showModal, setShowModal] = useState(false);

  const { data, setData, loading } = useFetch(async () => {
  ...
  const handleCreated = (milestone) => {
    setData(d => ({ ...d, milestones: [...d.milestones, milestone].sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)) }));
    setShowModal(false);
  };
