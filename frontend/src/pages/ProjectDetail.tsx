...
import type { Milestone, Project, ProjectFile, Task, TaskStatus, Ticket } from '../types';
...
type Tab = 'board' | 'list' | 'tickets' | 'milestones' | 'files' | 'members';
const TABS: Tab[] = ['board', 'list', 'tickets', 'milestones', 'files', 'members'];
...
export default function ProjectDetail() {
  ...
  const { data: detail, setData: setDetail, loading } = useFetch<ProjectDetailData>(async () => {
  ...
  const handleTaskSaved = (task: Task, isNew: boolean) => {
    setDetail(d => d && ({ ...d, tasks: isNew ? [...d.tasks, task] : d.tasks.map(t => t._id === task._id ? task : t) }));
  ...
  const handleTaskDelete = async (taskId: string) => {
    ...
    setDetail(d => d && ({ ...d, tasks: d.tasks.filter(t => t._id !== taskId) }));
  };

  const handleStatusChange = async (taskId: string, newStatus: TaskStatus) => {
    const { data } = await api.put(`/tasks/${taskId}`, { status: newStatus });
    setDetail(d => d && ({ ...d, tasks: d.tasks.map(t => t._id === taskId ? data : t) }));
  };
  ...
  const handleAddMember = async (e: FormEvent) => {
    ...
    try {
      ...
      setDetail(d => d && ({ ...d, project: data })); setMemberEmail('');
    } catch (err: any) { setMemberError(err.response?.data?.message || 'Failed to add member'); }
  };
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setDetail(d => d && ({ ...d, project: { ...d.project, files: [...d.project.files, data] } }));
    } catch (err: any) { setFileError(err.response?.data?.message || 'Failed to upload file'); }
  ...
  const handleFileDelete = async (fileId: string) => {
    ...
    setDetail(d => d && ({ ...d, project: { ...d.project, files: d.project.files.filter(f => f._id !== fileId) } }));
  };

  const handleTicketSaved = (ticket: Ticket, isNew: boolean) => {
    setDetail(d => d && ({ ...d, tickets: isNew ? [ticket, ...d.tickets] : d.tickets.map(t => t._id === ticket._id ? ticket : t) }));
  ...
  const handleTicketDelete = async (ticketId: string) => {
    ...
    setDetail(d => d && ({ ...d, tickets: d.tickets.filter(t => t._id !== ticketId) }));
  };

  const handleMilestoneSaved = (milestone: Milestone, isNew: boolean) => {
    setDetail(d => d && ({
      ...d,
  ...
  const handleMilestoneDelete = async (milestoneId: string) => {
    ...
    setDetail(d => d && ({ ...d, milestones: d.milestones.filter(m => m._id !== milestoneId) }));
  };
  ...
  const handleMilestoneToggle = async (milestone: Milestone) => {
    ...
    setDetail(d => d && ({ ...d, milestones: d.milestones.map(m => m._id === milestone._id ? data : m) }));
  };

  const milestoneStatus = (m: Milestone) => m.completed ? 'completed' : (new Date(m.dueDate) < new Date() ? 'overdue' : 'upcoming');
