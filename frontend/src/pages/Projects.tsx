...
const STATUSES: ('all' | ProjectStatus)[] = ['all', 'active', 'on-hold', 'completed', 'archived'];

export default function Projects() {
  const { data, setData, loading } = useFetch<Project[]>(() => api.get('/projects').then(r => r.data), []);
  const projects = data || [];
  const [showModal, setShowModal] = useState(false);
  const [searchParams, setSearchParams] = useSearchParams();
  const statusParam = searchParams.get('status') as ProjectStatus | null;
  const filter: 'all' | ProjectStatus = statusParam && STATUSES.includes(statusParam) ? statusParam : 'all';

  const handleCreated = (project: Project) => { setData(prev => [project, ...(prev || [])]); setShowModal(false); };
